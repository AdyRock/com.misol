...
class MyApp extends Homey.App
{
	...
	async onInit()
	{
		...
		this.alarmleakTrueTrigger = this.homey.flow.getDeviceTriggerCard('alarm_leak_true');
		this.alarmleakTrueTrigger.registerRunListener(async (args, state) =>
		{
			return state.value === true;
		});
		...
		const requestListener = (request, response) =>
		{
			...
			request.on('end', () =>
			{
				try
				{
					...
					// Get the IP address of the sending system
					let remoteIP = request.headers['x-forwarded-for'] ||
					...
					if (remoteIP)
					{
						...
						this.updateLog(`Data received from IP: ${remoteIP}`, 1);
					}
	...
	async getIOTDevices(gatewayIP)
	{
		if (!gatewayIP)
		{
			this.updateLog("Gateway IP is not set", 0);
			return;
		}
			...
			}, (res) =>
			{
				...
				res.on('end', () =>
				{
					try
					{
						...
						{
							device.gatewayIP = gatewayIP;
						});

						this.updateLog(`IOT Device List: ${JSON.stringify(deviceList)}`);
						resolve(deviceList);
					}
					catch (err)
					{
						this.updateLog(`Error parsing IOT Device List: ${err.message}`, 0);
						reject(err);
					}
				});
			}).on('error', (err) =>
			{
				this.updateLog(`Error fetching IOT Device List: ${err.message}`, 0);
				reject(err);
			}).end();
		});
	}
	...
	async getIOTDeviceList()
	{
		const devicePromises = this.gateways.map(async gateway =>
		{
			try
			{
				const devices = await this.getIOTDevices(gateway.ipAddress);
				return devices;
			} catch (err)
			{
				this.updateLog(`Error getting IOT Device List for ${gateway.ipAddress}: ${err.message}`, 0);
	...
	async getIOTDeviceStatus(address, model, id)
	{
		if (!address || !model || !id)
		{
			this.updateLog("Invalid parameters", 0);
			return;
		}
		...
		return new Promise((resolve, reject) =>
		{
			...
			req.on('error', (err) =>
			{
				this.updateLog(`Error fetching IOT Device Status: ${err.message}`, 0);
				reject(err);
			});
	...
	async setIOTDeviceOnOff(address, model, id, value)
	{
		if (!address || !model || !id)
		{
			this.updateLog("Invalid parameters", 0);
